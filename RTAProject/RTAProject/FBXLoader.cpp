...
namespace FBXLoader
{
		...
		std::vector< FbxNode* >& fbx_joints)
	{
		...
		for (int i = 0; i < numObjs; i++)
		{
			...
			{
				FbxNodeAttribute* nodeAttr = obj->GetNodeAttributeByIndex(i);
...
*/
		...
		std::vector< FbxNode* >& fbx_joints)
	{
		...
		{
			FbxNode* temp = fbx_joints[i]->GetParent();

			//for (size_t i = 0; i < temp->GetNodeAttributeCount(); i++)
			//{
			//	FbxNodeAttribute* nodeAttr = temp->GetNodeAttributeByIndex(i);
			//	if (nodeAttr->GetAttributeType() == )
			//		{
			//}
			//
			//transformHierarchy[i]. = temp
...
/*	
		...
		std::vector< unsigned int >& control_point_indices)
	{
		...
		for (size_t polyIndex = 0; polyIndex < fbx_mesh->GetPolygonCount(); polyIndex++)
		{
			for (size_t vertIndex = 0;  vertIndex < fbx_mesh->mPolygons.Size();  vertIndex++)
			{
				...
				vert.uvw[1] = 1- UVs->GetAt(uVIndex).mData[1]; // vert
				vert.uvw[2] = 0; // w
				...
				//		if (mesh.verts[i].xyz[j] == vert.xyz[j] && mesh.verts[i].xyz[j] == vert.xyz[j])
				//			break;
				//		else if (i )
				//			mesh.verts = vert;
