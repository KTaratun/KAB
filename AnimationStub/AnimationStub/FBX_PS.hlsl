...
float4 main(P_IN input) : SV_TARGET
{
	...
	if (finalRatio > 0.0f)
	{
		float3 reflectionVec = reflect(spotDir, bumpMap);//spotDir - ((spotDir + spotDir) + (input.normal * 2 * (dot(input.normal, -spotDir))));// normalize(2 * finalRatio * bumpNormal + spotDir);
